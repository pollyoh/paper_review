...
5. **논문 원본 이미지 활용**: 논문 경로에 PDF와 함께 PNG 파일(Figure 1.png, Figure 2.png 등)이 존재하면, 이는 원논문에서 추출한 그림이다. 텍스트나 mermaid만으로는 전달이 어렵고 **해당 이미지가 설명에 실질적으로 도움이 되는 경우에만** 보고서에 포함한다. 모든 이미지를 무조건 나열하지 말 것.
   ...
   - 삽입 형식: `![Figure_N 설명](../paper/{논문폴더명}/Figure N.png)` (report/ 기준 상대경로)
   - **그림 설명 추가**: 삽입된 각 이미지 바로 아래에 `> **Figure N**: ...` 형식의 인용 블록으로 상세 설명을 추가한다. 설명에는 축(X축/Y축)의 의미, 색상/기호의 구분, 핵심적으로 관찰해야 할 패턴 등 독자가 그림을 해석하는 데 필요한 정보를 포함한다.
...
3. 리뷰 페이지에서 다음이 정상 렌더링되는지 확인:
  - 표(table)
  - mermaid 다이어그램
  - KaTeX 수식 (인라인은 `$...$`, 블록은 `$$...$$`; `\(...\)`는 remark-math에서 인식되지 않으며 `lint_report.py`가 거절한다)
  - 주석 토글 (접이식)
  - TOC 사이드바
  - "목록으로" 네비게이션
